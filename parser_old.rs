...
impl parser {
	...
	fn parse_aexpr(tokens: &[Token], pos: &mut usize) -> Option<ASTNode> {
	...
	pub fn parse_comp_expr(tokens: &[Token], pos: &mut usize) -> Option<ASTNode> {
	    let mut node = parse_aexpr(tokens, pos)?;
	            ...
	            TokenType::Geq | TokenType::Leq | TokenType::Gre | TokenType::Les | TokenType::Eqv => {
	                ...
	                *pos += 1;
	                node = ASTNode::Op {
	                    op,
	                    left: Box::new(node),
	                    right: Box::new(parse_aexpr(tokens, pos)?),
	                };
	            }
	...
	pub fn parse_logic_expr(tokens: &[Token], pos: &mut usize) -> Option<ASTNode> {
	    let mut node = parse_comp(tokens, pos)?;
	            ...
	            TokenType::And | TokenType::Or => {
	                ...
	                *pos += 1;
	                node = ASTNode::Op {
	                    op,
	                    left: Box::new(node),
	                    right: Box::new(parse_comp(tokens, pos)?),
	                };
	            }
